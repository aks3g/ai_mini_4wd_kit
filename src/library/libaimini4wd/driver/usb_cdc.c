...
static void _usb_cdc_set_interrupt_data(void);
static void _usb_cdc_reset_internal(void);

/*--------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------*/
static uint8_t sInterruptInBuf0[8];
static uint8_t sInterruptInBuf1[8];
...
static uint8_t sTxBuf[64];
static uint8_t sRxBuf[64];
...
{
	size_t i = 0;
	while (!samd51_uart_fifo_is_empty(fifo)) {
		sTmpBuf[i++] = samd51_uart_fifo_dequeue(fifo);
	}
	
	if (i != 0) {
		samd51_usb_transfer_bulk_in(1, sTmpBuf, i);
	} 
}
...
/*--------------------------------------------------------------------------*/
static void _usb_cdc_standby(void)
{
	sCdcLinkUp = 0;
}
